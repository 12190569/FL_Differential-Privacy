...
class PersonalizedDPClient(fl.client.NumPyClient):
    def __init__(self, client_id: int):
        self.client_id = client_id
        self.client_type = "personalized"
        ...
        print(f"🚀 Cliente Personalized {client_id} inicializado")
    ...
    def fit(self, parameters, config):
        """Treinamento com Differential Privacy personalizado"""
        self.set_parameters(parameters)
        
        # 🆕 ε personalizado vindo do servidor
        epsilon = config.get("epsilon", 8.0)
        delta = config.get("delta", 1e-5)
        clip_norm = config.get("clip_norm", 2.0)
        learning_rate = config.get("learning_rate", 0.001)
        epochs = config.get("epochs", 1)
        round_idx = config.get("round", 0)
        personalization_reason = config.get("personalization_reason", "N/A")
        ...
        print(f"🔒 Cliente {self.client_id} - R{round_idx}: "
              f"ε={epsilon:.1f} (Personalized: {personalization_reason}), "
              f"Loss={avg_loss:.4f}")
    ...
    def apply_dp(self, epsilon: float, delta: float, clip_norm: float):
        """Aplica Differential Privacy com clipping e ruído"""
        if epsilon <= 0:
            return 0.0
...
def main():
    ...
    try:
        client = PersonalizedDPClient(client_id)
        print(f"🌐 Conectando cliente {client_id} ao servidor...")
        fl.client.start_client(
            server_address="0.0.0.0:8082",  # 🆕 Porta 8082
