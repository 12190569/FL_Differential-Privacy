...
class AdaptiveDPClient(fl.client.NumPyClient):
    def __init__(self, client_id: int):
        self.client_id = client_id
        self.client_type = "adaptive"
        ...
        print(f"🚀 Cliente Adaptive {client_id} inicializado")
    ...
    def fit(self, parameters, config):
        ...
        # Obter configurações do servidor (agora adaptativas!)
        ...
        schedule_type = config.get("schedule_type", "adaptive")
        ...
        print(f"🔒 Cliente {self.client_id} - R{round_idx}: "
              f"ε={epsilon:.1f} ({schedule_type}), Loss={avg_loss:.4f}, LR={learning_rate}")
...
def main():
    ...
    try:
        client = AdaptiveDPClient(client_id)
        print(f"🌐 Conectando cliente {client_id} ao servidor...")
        fl.client.start_client(
            server_address="0.0.0.0:8081",  # 🆕 Porta diferente!
