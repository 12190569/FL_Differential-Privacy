...
echo "=== EXPERIMENTO DP-SGD ==="

# Configurações
PORT=8083  # 🆕 Porta diferente
TIMEOUT=600
...
rm -f metrics_dp-sgd.json server.log client_*.log
sleep 2
...
# Iniciar servidor
echo "🚀 Iniciando servidor DP-SGD na porta ${PORT}..."
...
# Iniciar clientes
echo "👥 Iniciando 4 clientes DP-SGD..."
for i in {0..3}; do
    echo "   Cliente $i (alocação de ε)..."
    python client.py $i > client_$i.log 2>&1 &
    sleep 1
done

echo "=========================================="
echo "📊 DP-SGD em andamento..."
...
echo "✅ Experimento DP-SGD concluído!"
echo "📈 Métricas: metrics_dp-sgd.json"

# Resultados
if [ -f "metrics_dp_sgd.json" ]; then
    python3 -c "
import json
with open('metrics_dp_sgd.json') as f:
    data = json.load(f)
print('📊 RESULTADOS DP-SGD:')
